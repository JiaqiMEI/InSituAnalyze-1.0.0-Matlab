function varargout = A12_1CharacteristicBand(varargin)
% A12_1CHARACTERISTICBAND MATLAB code for A12_1CharacteristicBand.fig
%      A12_1CHARACTERISTICBAND, by itself, creates a new A12_1CHARACTERISTICBAND or raises the existing
%      singleton*.
%
%      H = A12_1CHARACTERISTICBAND returns the handle to a new A12_1CHARACTERISTICBAND or the handle to
%      the existing singleton*.
%
%      A12_1CHARACTERISTICBAND('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in A12_1CHARACTERISTICBAND.M with the given input arguments.
%
%      A12_1CHARACTERISTICBAND('Property','Value',...) creates a new A12_1CHARACTERISTICBAND or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before A12_1CharacteristicBand_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to A12_1CharacteristicBand_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help A12_1CharacteristicBand

% Last Modified by GUIDE v2.5 01-Mar-2018 20:53:53

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @A12_1CharacteristicBand_OpeningFcn, ...
                   'gui_OutputFcn',  @A12_1CharacteristicBand_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before A12_1CharacteristicBand is made visible.
function A12_1CharacteristicBand_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to A12_1CharacteristicBand (see VARARGIN)
...
% Choose default command line output for A12_1CharacteristicBand
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes A12_1CharacteristicBand wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = A12_1CharacteristicBand_OutputFcn(hObject, eventdata, handles) 
...
if ~isempty(get(handles.edit1,'String'))
    ...
    if (mod(a,2)==1)
        aa=find(Xaxis==(a+1));
        aaa=mean(Data2DP(:,(aa-2):(aa+1)),2);
    else
        aa=find(Xaxis==a);
        aaa=mean(Data2DP(:,(aa-2):(aa+2)),2);
    end
    axes(handles.axes2);
...
if ~isempty(get(handles.edit2,'String'))
    ...
    if (mod(a,2)==1)
        aa=find(Xaxis==(a+1));
        bbb=mean(Data2DP(:,(aa-2):(aa+1)),2);
    else
        aa=find(Xaxis==a);
        bbb=mean(Data2DP(:,(aa-2):(aa+2)),2);
    end
    axes(handles.axes3);
...
if ~isempty(get(handles.edit3,'String'))
    ...
    if (mod(a,2)==1)
        aa=find(Xaxis==(a+1));
        ccc=mean(Data2DP(:,(aa-2):(aa+1)),2);
    else
        aa=find(Xaxis==a);
        ccc=mean(Data2DP(:,(aa-2):(aa+2)),2);
    end
    axes(handles.axes4);
...
if ~isempty(get(handles.edit4,'String'))
    ...
    if (mod(a,2)==1)
        aa=find(Xaxis==(a+1));
        ddd=mean(Data2DP(:,(aa-2):(aa+1)),2);
    else
        aa=find(Xaxis==a);
        ddd=mean(Data2DP(:,(aa-2):(aa+2)),2);
    end
    axes(handles.axes5);
...
if ~isempty(get(handles.edit5,'String'))
    ...
    if (mod(a,2)==1)
        aa=find(Xaxis==(a+1));
        eee=mean(Data2DP(:,(aa-2):(aa+1)),2);
    else
        aa=find(Xaxis==a);
        eee=mean(Data2DP(:,(aa-2):(aa+2)),2);
    end
    axes(handles.axes10);
...
if ~isempty(get(handles.edit6,'String'))
    ...
    if (mod(a,2)==1)
        aa=find(Xaxis==(a+1));
        fff=mean(Data2DP(:,(aa-2):(aa+1)),2);
    else
        aa=find(Xaxis==a);
        fff=mean(Data2DP(:,(aa-2):(aa+2)),2);
    end
    axes(handles.axes9);
...
if ~isempty(get(handles.edit7,'String'))
    ...
    if (mod(a,2)==1)
        aa=find(Xaxis==(a+1));
        ggg=mean(Data2DP(:,(aa-2):(aa+1)),2);
    else
        aa=find(Xaxis==a);
        ggg=mean(Data2DP(:,(aa-2):(aa+2)),2);
    end
    axes(handles.axes8);
...
if ~isempty(get(handles.edit8,'String'))
    ...
    if (mod(a,2)==1)
        aa=find(Xaxis==(a+1));
        hhh=mean(Data2DP(:,(aa-2):(aa+1)),2);
    else
        aa=find(Xaxis==a);
        hhh=mean(Data2DP(:,(aa-2):(aa+2)),2);
    end
    axes(handles.axes7);
