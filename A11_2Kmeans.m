function varargout = A11_2Kmeans(varargin)
% A11_2KMEANS MATLAB code for A11_2Kmeans.fig
%      A11_2KMEANS, by itself, creates a new A11_2KMEANS or raises the existing
%      singleton*.
%
%      H = A11_2KMEANS returns the handle to a new A11_2KMEANS or the handle to
%      the existing singleton*.
%
%      A11_2KMEANS('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in A11_2KMEANS.M with the given input arguments.
%
%      A11_2KMEANS('Property','Value',...) creates a new A11_2KMEANS or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before A11_2Kmeans_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to A11_2Kmeans_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help A11_2Kmeans

% Last Modified by GUIDE v2.5 19-Jan-2018 12:42:33
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @A11_2Kmeans_OpeningFcn, ...
                   'gui_OutputFcn',  @A11_2Kmeans_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before A11_2Kmeans is made visible.
function A11_2Kmeans_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to A11_2Kmeans (see VARARGIN)

% Choose default command line output for A11_2Kmeans
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes A11_2Kmeans wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = A11_2Kmeans_OutputFcn(hObject, eventdata, handles) 
...
[cluster_idx, cluster_center] = kmeans(mCombine,Kn,'distance','sqEuclidean', 'Replicates',3);     %%%Kn=5
...
PurgeStr = A11_2Kmeans;
set(A11_2Kmeans,'Visible','off');
close gcf                             %%%shutdown needless window
PurgeAfter = A03_2Purge(handles.figure1);
...
close gcf
set(A10_2BandCombine,'Visible','on');
